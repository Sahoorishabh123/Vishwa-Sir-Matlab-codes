function [norm_Ag2] = beam_func_fill(r,Z2,phi,l2,n2,w,fil)
...
PMg2 = G2(r,Z2).*A2(r,Z2).*PHIM2(phi).*PSI2(Z2).*exp(-1i*0).*exp(1i*2*k2*fil*lambda);
PMg2_d = PMg2.*exp(1i*2*k2*surface);
% PM2 = G2(r,Z2).*A2(r,Z2).*PHIM2(phi).*PSI2(Z2).*exp(-1i*0);
Sg2 = P2+PMg2_d; %.*exp(1i*2*k*3);
% S2  = P2+PM2;
Ag2=abs(Sg2).^2;
% A2= abs(S2).^2;
